    ...
    if (this.selectedText) {
      ...
      if (selectedTextIndex !== -1) {
        ...
        const contextStart = Math.max(
          0,
          selectedTextIndex - Math.floor(maxContextLength / 2)
        );
        const contextEnd = Math.min(
          cleanTextContent.length,
          selectedTextIndex +
            this.selectedText.length +
            Math.floor(maxContextLength / 2)
        );
        contextText = cleanTextContent
        ...
        if (contextEnd < cleanTextContent.length)
        ...
        const walker = document.createTreeWalker(
          ...
          NodeFilter.SHOW_TEXT,
          {
            acceptNode: (node) => {
              ...
              if (
                ["script", "style", "noscript", "svg", "canvas"].includes(
                  tagName
                )
              ) {
